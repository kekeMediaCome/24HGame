...
import com.wolve.game24h.navigator.FixFragmentNavigator;
...
public class VNNavGraphBuilder {
    public static void build(FragmentActivity activity, FragmentManager childFragmentManager, NavController controller, int containerId) {
        ...
        while (iterator.hasNext()) {
            ...
            if (node.isFragment) {
                ...
                destination.setId(node.id);
                destination.setComponentName(new ComponentName(VNAppGlobals.getApplication().getPackageName(), node.className));
                destination.addDeepLink(node.pageUrl);
                navGraph.addDestination(destination);
            }
