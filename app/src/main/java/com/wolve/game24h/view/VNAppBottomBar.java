...
public class VNAppBottomBar extends BottomNavigationView {
    private static int[] sIcons = new int[]{R.drawable.icon_tab_find, R.drawable.icon_tab_accelerate, R.drawable.icon_tab_mine};
    private BottomBar config;

    public VNAppBottomBar(Context context) {
        this(context, null);
    }

    public VNAppBottomBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    @SuppressLint("RestrictedApi")
    public VNAppBottomBar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        config = VNAppConfig.getBottomBarConfig();
    ...
    private int getItemId(String pageUrl) {
        Destination destination = VNAppConfig.getDestConfig().get(pageUrl);
        if (destination == null)
            return -1;
